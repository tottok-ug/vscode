...
import Marked = require('vs/languages/hatena/common/marked');
...
export class HatenaWorker extends AbstractModeWorker {
	...
	public getEmitOutput(resource: URI, absoluteWorkersResourcePath: string): WinJS.TPromise<Modes.IEmitOutput> { // TODO@Ben technical debt: worker cannot resolve paths absolute
		...
		let highlighter = function(code: string, lang: string, callback?: (error: Error, result: string) => void) {
			...
			let mimeForLang = modesRegistry.getModeIdForLanguageName(lang) || lang || HatenaWorker.DEFAULT_MODE;
		...
		return new WinJS.Promise((c, e) => {
			...
			// Render hatena file contents to HTML
			Marked.marked(model.getValue(), {
				gfm: true, // GitHub flavored hatena
				renderer: renderer,
				...
				let head = [
					...
					(cssLinks.length === 0) ? '<link rel="stylesheet" href="' + absoluteWorkersResourcePath + '/hatena.css" type="text/css" media="screen">' : '',
					(cssLinks.length === 0) ? '<link rel="stylesheet" href="' + absoluteWorkersResourcePath + '/tokens.css" type="text/css" media="screen">' : '',
					(this.theme === Theme.LIGHT) ? HatenaWorker.LIGHT_SCROLLBAR_CSS : (this.theme === Theme.DARK) ? HatenaWorker.DARK_SCROLLBAR_CSS : HatenaWorker.HC_BLACK_SCROLLBAR_CSS,
					cssLinks.map((style) => {
